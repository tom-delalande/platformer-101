...
object Assets {
    ...
    val textures by lazy {
        Sprite.sprites.map {
            it.value to cachedTextures.getOrPut(it.value.texture) { Raylib.loadTexture(it.value.texture) }
        }
    }

    val sounds by lazy {
        Audio.audio.map { it.value to Raylib.loadSound(it.value.file) }
    }
